...
function write_log_ws($cadena,$tipo)
{
...
class WS_PS{
		...
		function getSKU($sku,$zone){
			...
			$pos = strpos($codigo, '-');
				if ($pos !== false) {
						...
						$arr_codes = explode('-',$codigo);
					$codigo = array($arr_codes[0],$arr_codes[1]);
		...
		public function buscaClienteNIF(){
			...
			$NIF = str_replace(' ','',$this->order->billing_documento_identificacion);
			...
			write_log_ws(json_encode($param),'buscaClienteNIF');
					...
					foreach($items as $item) { 
						...
						$product_sku = $product->get_sku();
							...
							switch($this->order->get_payment_method()){
								...
								*/
								case 'paypal':$forma_pago = '14';$txid=$this->order->id;break;	
								case 'redsys':$forma_pago = '13';$txid=$this->order->id;break;
								case 'realex_redirect':$forma_pago = '13';$txid=$this->order->id;break;
							...
							$codigos = $this->getSKU($product_sku,'ES');
							...
							write_log_ws(json_encode($param),'altaCompra');
							...
							if(isset($res['ok'])&&$res['ok']==1){
								...
								$cupones = '';
								 if($coupons = $this->order->get_used_coupons() ) {
							        ...
							        write_log_ws(json_encode($param),'detalleVenta');
							        ...
							        write_log_ws(json_encode($param),'encuestaVenta');
...
function logErrorPs($msj){
	
	
}


function callWSPelostop($order_id){
	
	global $woocommerce, $post;
	
	$ws = new WS_PS($order_id);
...
function pelostop_woocommerce_order_status_completed( $order_id ) {
   	callWSPelostop($order_id,'');
}
add_action( 'woocommerce_order_status_completed', 'pelostop_woocommerce_order_status_completed', 10, 1 );
